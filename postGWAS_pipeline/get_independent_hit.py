...
if len(sys.argv) != 2:
    print("Usage: python get_independent_hit.py stem")
    sys.exit(0)
stem = sys.argv[1]
...
for chrom in range(1, 23):
    ...
    if os.path.exists(ld_f):
        ld_dict[chrom] = pd.read_csv(ld_f, sep=" ", comment="#")
        # print status
...
for filter_snp_f in filter_snp_fs:
    ...
    while filter_snp_df.shape[0] > 0:
        # print column titles
        print("column names:" + str(filter_snp_df.columns))
        # get row with the lowest p-value
        min_p_row = filter_snp_df.loc[filter_snp_df["LOG10P"].idxmin()]
        # record the index snp
        index_snps_df = pd.concat([index_snps_df, min_p_row])
        # get list of snps that fits the LD threshold
        ld_df = ld_dict[min_p_row["CHR"]]
        ld_df = ld_df.loc[ld_df["SNP1"] == min_p_row["SNP"]]
        ld_df = ld_df.loc[ld_df["R2"] >= ld_threshold]
        ld_snps = list(ld_df["SNP2"]).append(min_p_row["SNP"])
    ...
    # save the index snps
    print("number of index snps: " + str(index_snps_df.shape[0]))
    index_snps_df.to_csv(filter_snp_f.replace("filtered", "clump"), sep="\t", index=False)
