...
do
    f_1e6=$(echo $summary_file | sed 's/^.*\///g' | sed 's/txt.gz/5e8_snp.txt/g')
    ...
    # run commanp to keep the rows where -log10(p) >= 7.301, run it in the backgournd
    (
        echo "retrieving 1e6 snps for $f_1e6...."
        zcat $summary_file | awk  '{if ($13 >= 7.301) print $0}' > ${out_dir}/${f_1e6}
        echo "$f_1e6 done"
    )&
done
